$(document).ready(function() {
    ...
    function init() {
        ...
        // Waiting for user input a date
        $("input").change(function() {
            let user_date = $( this ).val();
            console.log(`CHANGED TO ${user_date}`);
            var time = getTime($(this).attr("id"), user_date);
            snoozeCurrentTab(time);
        })
    ...
    function getTime(timeName, userTime) {
        console.log('>>> 2 <<<<<')
        ...
        switch(timeName) {
            ...
            case "datepicker":
                console.log('>>> 5 <<<<<');
                console.log('user_date:', userTime);
                let days = parseInt(userTime.split('/')[1]);
                today = result.getDate();
                delta = days - today
                console.log(`days: ${days}, today: ${today}, delta: ${delta}`);
